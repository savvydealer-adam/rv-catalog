...
**STL RV sync (2026-05-08):** `scripts/sync_to_stl_rv.py` mirrors rv-catalog SQLite → STL RV Supabase (`blcoiejnzrdxjwgxmlui`). Diff-based upsert keyed by natural keys (mfr name; model = mfr_id+name+year; floorplan = model_id+code; image = model_id+source_url). Never deletes. Reads `SUPABASE_SERVICE_KEY` from env. Idempotent. First apply landed 81 mfrs + 1,317 models + 3,649 floorplans into STL RV; STL RV mfrs went 16 → 97, models 141 → 1,458, floorplans 553 → 4,202. Run with `python scripts/sync_to_stl_rv.py` (apply) or `--dry-run` (preview, --slug `<x>` to scope). `--skip-images` is currently the default-needed flag — the kb_images sync hits a `local_path NOT NULL` constraint on STL RV's table because STL RV's existing image pipeline writes a synthesized `images/<Display>/<Model>/exterior_NN.webp` path. Either drop the NOT NULL constraint on STL RV side or have the sync synthesize the same path; **next session.**

**Next session — floorplans:** the sync delivered floorplan rows but `floorplans.image_url` is still empty for the 3,649 new ones. STL RV's existing `scripts/update_kb_floorplans.py` populates that column from a GCS bucket (`images/<Display>/<Model>/exterior_NN.webp` convention). Plan: (a) decide whether rv-catalog images flow into STL RV's GCS bucket via the sync, or stay as direct OEM source URLs; (b) handle the kb_images NOT NULL gap; (c) re-run sync without --skip-images. Touch only after image-flow direction is decided.

**Residual follow-ups (post-2026-04-20):**
- **Spec-table hit rate (2026-05-07 measurement):** Re-measured jayco/winnebago/tiffin — the prompt hint (commit 3ade128) DID bite at the floorplan level. Floorplan post-hint coverage: jayco dry_weight 30.0%, gvwr 27.8%; winnebago dry_weight 30.4%; tiffin gvwr 48.6%. Where coverage stays at 0% (tiffin dry_weight, winnebago gvwr) the source site genuinely doesn't publish those fields — already documented. **What was missing was a separate bug:** the model-level Gemini prompt doesn't ask for `dry_weight_lbs_min/max`, `gvwr_lbs_min/max`, or `bathroom_count_min/max`, so model rows had 0% coverage on those even when their floorplans had 30-85% coverage. Fix shipped: `_persist` now rolls up floorplan MIN/MAX into the model row via COALESCE (preserves Gemini ranges, only fills NULLs). One-shot backfill across the existing DB filled bath on 504 model rows, dry_weight on 623, gvwr on 690, plus partial backfill on length/sleep/slide where Gemini missed them at the model level. DB backed up at `data/rv_catalog.db.bak.rollup-20260507-101314`.
...
- **IPRoyal retired 2026-05-07.** Replaced with the Tailscale home-proxy pool comp-dashboard already uses (main PC on Comcast + Dell on Spectrum, 2 `proxy.py` instances). Reads `RV_HOME_PROXY_POOL` first, falls back to `CD_HOME_PROXY_POOL` so a single env var on the host serves both projects. Sticky per slug (CRC32(slug) % len(pool)), retry rotates, TCP-probed with 60s TTL cache, falls through to direct egress when all entries dead. Playwright runs proxyless (Chromium proxy-auth path is fragile). The old "IPRoyal blank-body" follow-up is moot — the home pool's never produced that failure mode on comp-dashboard.

**Infra shipped:** Tailscale home-proxy pool (2 nodes, sticky per slug), Qwen3:32b recon pipeline (`scripts/qwen_site_recon.py`), per-brand configs with `model_path_patterns` and `force_stealth` escape hatches, **stealth fetcher** (puppeteer-real-browser, local PC, bypasses Cloudflare/Akamai without proxy).
