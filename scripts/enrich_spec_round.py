"""Re-scrape brands most likely to benefit from the 2026-04-20 extractor
fixes (spec-table prompt hint + image-CDN + HTML-entity unescape). Runs up
to 3 at a time. INSERT OR IGNORE keeps prior rows.

Prints per-brand before/after snapshot for model_count, floorplan_count,
image_count AND spec-completion rates for key floorplan fields.
"""
...
TARGETS = [
    ("tiffin", "0/40 dry weight; 16/40 gvwr (table format visible on page)"),
    ("winnebago", "0/59 gvwr"),
    ("roadtrek", "0/20 dry + 0/20 gvwr; 8 total images"),
    ("ibex", "4/19 sleep; 18 total images"),
    ("sabre", "2/10 sleep; 15 total images"),
    ("flagstaff-rv", "0/40 sleep; 10 total images"),
]
...
async def main():
    ...
    out = Path(__file__).resolve().parent / "logs" / f"enrich_spec_{int(time.time())}.json"
